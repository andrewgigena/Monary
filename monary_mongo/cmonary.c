...
enum {
  ...
  TYPE_FLOAT64 = 12,
  TYPE_DATE =
      13, // BSON date-time, seconds since the UNIX epoch (uint64 storage)
  TYPE_TIMESTAMP =
      14, // BSON timestamp - UNIX timestamp and increment (uint64 storage)
  ...
  TYPE_SIZE =
      19, // data size of a string, symbol, binary, or bson object (uint32)
  TYPE_LENGTH =
      20, // length of string (character count) or num elements in BSON (uint32)
...
mongoc_client_t *monary_connect(const char *uri, const char *pem_file,
  ...
  if (mongoc_uri_get_ssl(mongo_uri)) {
    mongoc_ssl_opt_t opts = {pem_file, pem_pwd,  ca_file,
                             ca_dir,   crl_file, weak_cert_validation};
    mongoc_client_set_ssl_opts(client, &opts);
    DEBUG("Setting SSL opts={%s, %s, %s, %s, %s, %i} \n", pem_file, pem_pwd,
          ca_file, ca_dir, crl_file, weak_cert_validation);
  }
...
#define MONARY_DEFINE_FLOAT_LOADER(FUNCNAME, NUMTYPE)                          \
  int FUNCNAME(const bson_iter_t *bsonit, monary_column_item *citem,           \
               int idx) {                                                      \
    NUMTYPE value;                                                             \
...
int64_t monary_query_count(mongoc_collection_t *collection,
  ...
  total_count = mongoc_collection_count(collection, MONGOC_QUERY_NONE,
...
monary_cursor *monary_init_query(mongoc_collection_t *collection,
  ...
  // create query cursor
  mcursor = mongoc_collection_find(collection, MONGOC_QUERY_NONE, offset, limit,
                                   0, &query_bson, fields_bson, NULL);
...
monary_cursor *monary_init_aggregate(mongoc_collection_t *collection,
  ...
  mcursor = mongoc_collection_aggregate(collection, MONGOC_QUERY_NONE, &pl_bson,
                                        NULL, NULL);
...
#define MONARY_SET_BSON_VALUE(TYPENAME, BTYPENAME, VKEY, STORED_TYPE,          \
                              CAST_TYPE)                                       \
  case TYPENAME:                                                               \
...
void monary_make_bson_value_t(bson_value_t *val, monary_column_item *citem,
  ...
  switch (citem->type) {
    ...
    MONARY_SET_BSON_VALUE(TYPE_INT16, BSON_TYPE_INT32, v_int32, int16_t,
                          int32_t)
    MONARY_SET_BSON_VALUE(TYPE_INT32, BSON_TYPE_INT32, v_int32, int32_t,
                          int32_t)
    MONARY_SET_BSON_VALUE(TYPE_INT64, BSON_TYPE_INT64, v_int64, int64_t,
                          int64_t)
    MONARY_SET_BSON_VALUE(TYPE_UINT8, BSON_TYPE_INT32, v_int32, uint8_t,
                          int32_t)
    MONARY_SET_BSON_VALUE(TYPE_UINT16, BSON_TYPE_INT32, v_int32, uint16_t,
                          int32_t)
    MONARY_SET_BSON_VALUE(TYPE_UINT32, BSON_TYPE_INT32, v_int32, uint32_t,
                          int32_t)
    MONARY_SET_BSON_VALUE(TYPE_UINT64, BSON_TYPE_INT64, v_int64, uint64_t,
                          int64_t)
    MONARY_SET_BSON_VALUE(TYPE_FLOAT32, BSON_TYPE_DOUBLE, v_double, float,
                          double)

    MONARY_SET_BSON_VALUE(TYPE_FLOAT64, BSON_TYPE_DOUBLE, v_double, double,
                          double)

    MONARY_SET_BSON_VALUE(TYPE_DATE, BSON_TYPE_DATE_TIME, v_datetime, int64_t,
                          int64_t)
  case TYPE_OBJECTID:
  ...
  case TYPE_TIMESTAMP:
    ...
    memcpy(&val->value.v_timestamp.increment,
           ((uint32_t *)citem->storage) + (2 * idx + 1), sizeof(uint32_t));
    break;
  case TYPE_STRING:
...
void monary_bson_from_columns(monary_column_item *columns, int row,
  ...
  if (depth >= MONARY_MAX_RECURSION) {
    DEBUG("Max recursive depth (%d) exceed on row: %d", MONARY_MAX_RECURSION,
          row);
    return;
  }
  for (i = col_start; i < col_end; i++) {
    ...
    if (!*(citem->mask + row)) {
      ...
      for (field = citem->field + name_offset;
           *(field + dot_idx) && (field[dot_idx] != '.'); dot_idx++)
      ...
      if (*(field + dot_idx)) {
        ...
        while (new_end < col_end) {
          ...
          if (strlen((columns + new_end)->field) > name_offset + dot_idx) {
            ...
            if (strncmp(field, (columns + new_end)->field + name_offset,
                        dot_idx) == 0) {
              new_end++;
              continue;
            }
        ...
        bson_append_document_begin(parent, citem->field + name_offset, dot_idx,
                                   &child);
...
void monary_insert(mongoc_collection_t *collection, monary_column_data *coldata,
  ...
  bulk_op =
      mongoc_collection_create_bulk_operation(collection, false, write_concern);
  ...
  DEBUG("Inserting %d documents with %d keys.", coldata->num_rows,
        coldata->num_columns);
  for (row = 0; row < coldata->num_rows; row++) {
    ...
    monary_bson_from_columns(coldata->columns, row, 0, coldata->num_columns,
                             &document, 0, 0);
    ...
    if (data_len > max_message_size || row == (coldata->num_rows - 1)) {
      ...
      if (mongoc_bulk_operation_execute(bulk_op, &reply, err)) {
        ...
        if (bson_iter_init_find(&bsonit, &reply, "writeErrors")) {
          i = monary_mask_failed_writes(&bsonit, id_data->columns->mask,
                                        num_processed);
          if (i != -1) {
